...
class QuestionService
{
    public function getQuestions(Request $request)
    {
        $Questions = Questions::select(
            ...
            ->where('exam_masters.id', $request->exam_type)
    ...
    public function questionVerify(Request $request)
    {
        ...
        $questionAnswersDB = Questions::with('questionOptions')
            ->whereIn('id', $getQuestionIds)->get();
        $getSubjectId = $questionAnswersDB->pluck('subject_id')->first();
        $totalMarks = 0;
        foreach ($questionAnswersDB->toArray() as $question) {
